      ...
      <template v-if="currentKey">
        ...
        <c-scrollbar trigger="hover" style="height: 100%">
          <div class="cpt-list-box">
            <div v-for="(element, index) in cptList" class="com-item" :key="index">
              <!-- 图片 -->
              <el-tooltip class="box-item" effect="light" :enterable="false" placement="right">
                <template #content>
                  <div class="toolTip-box">
                    <!-- 效果图 -->
                    <img :src="getAssetsMaterialFile(currentKey, cptOfImg[currentKey][element.cptName].url)
                      " alt="效果图" />
                    <div class="layout-box"> 建议位置：{{ layoutDic[element.layout] }}</div>
                  </div>
                </template>
                <div class="item-img-box" :style="{
...
// 点击组件 添加模块
const { resumeJsonNewStore, pushComponent } = appStore.useResumeJsonNewStore;
...
const selectStoreModel = (item: any) => {
  const updateData = {
    model: item.model,
    optionsName: item.cptOptionsName,
