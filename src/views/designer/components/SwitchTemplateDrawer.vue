<template>
    <el-drawer class="switch-template-drawer" :model-value="drawerSwitchVisible" :modal="true" :lock-scroll="false"
        :close-on-click-modal="true" :direction="direction" :destroy-on-close="true" :size="400" @close="closeSwitchDrawer" title="请选择模板" >
        <template #header>
            <h1 class="title">请选择模板</h1>
        </template>
        <!-- 内容区 -->
        <div class="drawer-content">
            <el-skeleton v-if="isShowSkeleton" :rows="15" animated></el-skeleton>
            <template v-else>
                <div v-for="(item, index) in templateList" :key="index" class="card-box">
                    ...
                    <div v-if="route.query.id == item.ID" class="current-mark">
                        <svg-icon icon-name="icon-duigou_kuai" color="green" size="16px"></svg-icon>
                    </div>
                    <!-- 遮罩层 -->
