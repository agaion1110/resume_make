<template>
  <div class="model-list-box">
    <div v-if="resumeJsonNewStore.COMPONENTS.length" class="model-ul">
      <draggable :list="resumeJsonNewStore.COMPONENTS" item-key="id" ghost-class="ghost" chosen-class="chosenClass"
        ...
        <template #item="{ element }">
          <div :class="[
            ...
            <!-- 是否添加模块 -->
            <div class="left">
              <div :class="['icon-box', { 'collapse-center': !leftShowStatus }]">
                <el-tooltip class="box-item" placement="right" effect="dark" :content="element.data.title"
                  :disabled="leftShowStatus">
                  <svg-icon :icon-name="element.data.iconfont ? element.data.iconfont : 'icon-xingquaihao'"
                    class-name="icon" :color="leftShowStatus ? '#c4c4c4' : '#00c091'" size="16px"></svg-icon>
                </el-tooltip>
              </div>
            ...
            <div v-show="leftShowStatus" class="right">
              <el-switch v-model="element.show" size="small" active-color="#00C091"></el-switch>
            </div>
          </div>
        </template>
      </draggable>
...
const { resumeJsonNewStore } = appStore.useResumeJsonNewStore; //  获得store中的相关数据
