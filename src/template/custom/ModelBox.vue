<template>
  <div v-if="item.show" :ref="(el) => setRefItem(el, item.keyId)" :class="[
    'material-model-box',
    { 'is-have-border': item.keyId === appStore.useSelectMaterialStore.cptKeyId }
  ]" @click="selectModel" @mouseover="handleMouseover" @mouseleave="handleMouseleave">
    <!-- 模块操作区域 -->
    <div v-show="hoverId === item.keyId" class="edit-box">
      ...
      <el-tooltip class="box-item" effect="dark" content="删除当前模块">
        <div class="delete" @click.stop="delModel">
          <svg-icon icon-name="icon-shanchu" class-name="icon icon-shanchu" color="#fff" size="18px"></svg-icon>
        </div>
      </el-tooltip>
    </div>
    <component :is="components[item.cptName]" :class="['mode-item', { isHover: hoverId === item.keyId }]"
      :model-data="item.data" :model-style="item.style" :style="getDynamicStyle(item)"></component>
  </div>
</template>
...
// 传统布局添加模块
const classicalAdd = () => {
  let index: number = resumeJsonNewStore.value.COMPONENTS.findIndex((item) => {
    item.keyId === props.item.keyId;
  });
